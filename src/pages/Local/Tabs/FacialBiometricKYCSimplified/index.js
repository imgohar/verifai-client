...
const FacialBiometricKYCSimplified = () => {
    let testKey =
        process.env.REACT_APP_LOCAL_FACIAL_BIOMETRIC_KYC_SIMPLIFIED_TEST_KEY;
    let liveKey =
        process.env.REACT_APP_LOCAL_FACIAL_BIOMETRIC_KYC_SIMPLIFIED_LIVE_KEY;
    const [value, setValue] = useState(
        'https://sahal.dev-net.co:8084/api/v1/face-kyc-simplified/face-register/frame-url?reference=USER_REFERENCE'
    );
    ...
    useEffect(() => {
        setValue(
            `https://sahal.dev-net.co:8084/api/v1/face-kyc-simplified/face-register/frame-url?reference=${userReference}`
        );
    ...
    const onSubmit = async (data) => {
        ...
        if (Object.values(data).every((field) => field.length > 0)) {
            ...
            await axios
                .get(
                    `https://sahal.dev-net.co:8084/api/v1/face-kyc-simplified/face-register/frame-url?reference=${data.userReference}`,
                    config
                )
                .then((res) => {
                    setShowLink(true);
