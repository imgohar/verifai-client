...
const SecureFaceLogin = () => {
    let testKey = process.env.REACT_APP_LOCAL_SECURE_FACE_LOGIN_TEST_KEY;
    let liveKey = process.env.REACT_APP_LOCAL_SECURE_FACE_LOGIN_LIVE_KEY;
    const [value, setValue] = useState(
        'https://sahal.dev-net.co:8084/api/v1/laas/face-register/frame-url?reference=USER_REFERENCE'
    );
    ...
    useEffect(() => {
        setValue(
            `https://sahal.dev-net.co:8084/api/v1/laas/face-register/frame-url?reference=${userReference}`
        );
    ...
    const onSubmit = async (data) => {
        ...
        if (Object.values(data).every((field) => field.length > 0)) {
            ...
            await axios
                .get(
                    `https://sahal.dev-net.co:8084/api/v1/laas/face-register/frame-url?reference=${data.userReference}`,
                    config
                )
                .then((res) => {
                    setShowLink(true);
    ...
    return (
        <div className="container">
            <h2 className="text-center">Secure Face Login</h2>
...
export default SecureFaceLogin;
