...
const DigitalSignature = () => {
    let testKey = process.env.REACT_APP_DEVELOP_DIGITAL_SIGNATURE_TEST_KEY;
    let liveKey = process.env.REACT_APP_DEVELOP_DIGITAL_SIGNATURE_LIVE_KEY;
    const [value, setValue] = useState(
        'https://develop.sahal.ai/api/v1/digital-signature/face-register/frame-url?reference=USER_REFERENCE&digital_signature=USER_FULL_NAME'
    );
    ...
    useEffect(() => {
        setValue(
            `https://develop.sahal.ai/api/v1/digital-signature/face-register/frame-url?reference=${userReference}&digital_signature=${fullName}`
        );
    ...
    const onSubmit = async (data) => {
        ...
        if (Object.values(data).every((field) => field.length > 0)) {
            ...
            await axios
                .get(
                    `https://develop.sahal.ai/api/v1/digital-signature/face-register/frame-url?reference=${data.userReference}&digital_signature=${data.fullName}`,
                    config
                )
                .then((res) => {
                    setShowLink(true);
