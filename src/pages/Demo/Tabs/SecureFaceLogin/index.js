...
const SecureFaceLogin = () => {
    let testKey = process.env.REACT_APP_DEMO_SECURE_FACE_LOGIN_TEST_KEY;
    let liveKey = process.env.REACT_APP_DEMO_SECURE_FACE_LOGIN_LIVE_KEY;
    const [value, setValue] = useState(
        'https://demo.sahal.ai/api/v1/laas/face-register/frame-url?reference=USER_REFERENCE'
    );
    ...
    useEffect(() => {
        setValue(
            `https://demo.sahal.ai/api/v1/laas/face-register/frame-url?reference=${userReference}`
        );
    ...
    const onSubmit = async (data) => {
        ...
        if (Object.values(data).every((field) => field.length > 0)) {
            ...
            await axios
                .get(
                    `https://demo.sahal.ai/api/v1/laas/face-register/frame-url?reference=${data.userReference}`,
                    config
                )
                .then((res) => {
                    setShowLink(true);
