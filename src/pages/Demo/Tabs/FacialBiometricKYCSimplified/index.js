...
const FacialBiometricKYCSimplified = () => {
    let testKey =
        process.env.REACT_APP_DEMO_FACIAL_BIOMETRIC_KYC_SIMPLIFIED_TEST_KEY;
    let liveKey =
        process.env.REACT_APP_DEMO_FACIAL_BIOMETRIC_KYC_SIMPLIFIED_LIVE_KEY;
    const [value, setValue] = useState(
        'https://demo.sahal.ai/api/v1/face-kyc-simplified/face-register/frame-url?reference=USER_REFERENCE'
    );
    ...
    useEffect(() => {
        setValue(
            `https://demo.sahal.ai/api/v1/face-kyc-simplified/face-register/frame-url?reference=${userReference}`
        );
    ...
    const onSubmit = async (data) => {
        ...
        if (Object.values(data).every((field) => field.length > 0)) {
            ...
            await axios
                .get(
                    `https://demo.sahal.ai/api/v1/face-kyc-simplified/face-register/frame-url?reference=${data.userReference}`,
                    config
                )
                .then((res) => {
                    setShowLink(true);
