...
const FacialBiometricKYC = () => {
    let testKey = process.env.REACT_APP_PROD_FACIAL_BIOMETRIC_KYC_TEST_KEY;
    let liveKey = process.env.REACT_APP_PROD_FACIAL_BIOMETRIC_KYC_LIVE_KEY;
    const [value, setValue] = useState(
        'https://production.sahal.ai/api/v1/face-kyc/face-register/frame-url?reference=USER_REFERENCE'
    );
    ...
    useEffect(() => {
        setValue(
            `https://production.sahal.ai/api/v1/face-kyc/face-register/frame-url?reference=${userReference}`
        );
    ...
    const onSubmit = async (data) => {
        ...
        if (Object.values(data).every((field) => field.length > 0)) {
            ...
            await axios
                .get(
                    `https://production.sahal.ai/api/v1/face-kyc/face-register/frame-url?reference=${data.userReference}`,
                    config
                )
                .then((res) => {
                    setShowLink(true);
